...
def classify_new_elements(new_selectors, steps_info, created_steps=None):
    ...
    if created_steps is None:
        created_steps = []  # Nếu không truyền, fallback về empty để tránh lỗi
    ...
    # SỬA: Thêm current_step (step cuối cùng trong steps_info) vào element_steps nếu action là click/double click và chưa tồn tại
    if steps_info:
    ...
    # Logic sắp xếp theo thứ tự trong DOM:
    # Connector của node sẽ đứng trước node đó
    # Ví dụ: connector_2_to_3, connector_2_to_9, element_2, connector_3_to_5, element_3, ...

    selector_idx = 0

    # Nhóm connectors theo node gốc (from_step)
    connectors_by_source = {}
    for conn in connector_steps:
        from_step = conn['from_step']
        if from_step not in connectors_by_source:
            connectors_by_source[from_step] = []
        connectors_by_source[from_step].append(conn)

    # Sắp xếp các connector trong mỗi nhóm theo to_step tăng dần
    for from_step in connectors_by_source:
        connectors_by_source[from_step].sort(key=lambda x: x['to_step'])

    # Sort element_steps theo step_num tăng dần để duyệt từ node nhỏ đến lớn
    element_steps = sorted(element_steps, key=lambda x: x['step_num'])

    # Duyệt qua các element step theo thứ tự tăng dần
    for elem_step in element_steps:
        step_num = elem_step['step_num']

        # Gán các connector có from_step = step_num (đã sort theo to_step)
        if step_num in connectors_by_source:
            for conn in connectors_by_source[step_num]:
