...
class Generator:
    ...
    def generate_code_for_step(self, url: str, step: Step, driver, step_created_elements=None,
        ...
        if obj:
            log(f"[FIND] Đang tìm object: '{obj}'")
            ...
            if retry_attempt == 0 and step_index is not None:
                ...
                log(f"[SAVE] Lưu {len(all_locators)} locators cho object. Sử dụng: {object_locator}")
            ...
            # Lần retry: Lấy locator tiếp theo từ fallback list
                ...
                if current_idx < len(locators):
                    ...
                    object_locator_type = detect_locator_type(object_locator)
                    log(f"[RETRY] Retry {retry_attempt}: Thử locator [{current_idx}]: {object_locator}")
                else:
                    log(f"Đã HẾT locators để fallback cho object!")
                    object_locator = "//not-found"
            ...
            log(f"[O] Object Locator ({object_locator_type}): {object_locator}")

        # TÌM RELATED OBJECT với hỗ trợ fallback (tương tự)
        if rel_obj:
            log(f"[FIND] Đang tìm related object: '{rel_obj}'")
            ...
            if retry_attempt == 0 and step_index is not None:
                ...
                log(f"[SAVE] Lưu {len(all_locators)} locators cho related object. Sử dụng: {related_locator}")
                ...
                if current_idx < len(locators):
                    ...
                    related_locator_type = detect_locator_type(related_locator)
                    log(f"[RETRY] Retry {retry_attempt}: Thử locator [{current_idx}]: {related_locator}")
                else:
                    log(f"Đã HẾT locators để fallback cho related object!")
                    related_locator = "//not-found"
            ...
            log(f"[R] Related Locator ({related_locator_type}): {related_locator}")
    ...
    def execute_python_action(self, driver, action, object_locator, related_locator, val, obj, rel_obj,
        ...
        def scroll_to_element(element):
            ...
            try:
                ...
                log(f"[WARNING] Không thể scroll đến element: {e}")
        ...
        def ensure_element_visible(element):
            ...
            try:
                ...
                if not is_displayed:
                    log("[WARNING] Element không hiển thị, thử scroll lại...")
                    driver.execute_script("arguments[0].scrollIntoView(true);", element)
                    time.sleep(0.3)
                
                return True
            except Exception as e:
                log(f"[ERROR] Không thể đảm bảo element visible: {e}")
                return False
        ...
        try:
            if action == "open":
                ...
                # Xử lý ESC linh hoạt
                if val_lower in ("esc", "escape"):
                    # 1) Thoát fullscreen API của website nếu đang bật
                    is_fullscreen = driver.execute_script("return !!document.fullscreenElement;")
                    ...
                    # 2) gửi ESC vào DOM như bình thường
                    active_elem = driver.switch_to.active_element
                    active_elem.send_keys(Keys.ESCAPE)
                    wait_for_page_load(driver)
                    return True
                
                if val_lower in ("f5", "refresh"):
                    log("Thực hiện REFRESH trang (F5)")
                    driver.refresh()
                    wait_for_page_load(driver)
                    return True

                # map phím bình thường
                key_map = {
                    "enter": Keys.ENTER, "tab": Keys.TAB, "space": Keys.SPACE,
                    "delete": Keys.DELETE, "backspace": Keys.BACKSPACE,
                ...
                if val == "right":
                    # Kéo cạnh phải vào trong (thu nhỏ chiều rộng)
                    actions.move_to_element(elem).move_by_offset(elem.size['width'] // 2, 0).click_and_hold().move_by_offset(-20, 0).release().perform()
                elif val == "left":
                    # Kéo cạnh trái vào trong (thu nhỏ chiều rộng)
                    actions.move_to_element(elem).move_by_offset(-elem.size['width'] // 2, 0).click_and_hold().move_by_offset(20, 0).release().perform()
                elif val in ["bottom", "down"]:
                    # Kéo cạnh dưới lên trên (thu nhỏ chiều cao)
                    actions.move_to_element(elem).move_by_offset(0, elem.size['height'] // 2).click_and_hold().move_by_offset(0, -20).release().perform()
                elif val in ["top", "up"]:
                    # Kéo cạnh trên xuống dưới (thu nhỏ chiều cao)
                    actions.move_to_element(elem).move_by_offset(0, -elem.size['height'] // 2).click_and_hold().move_by_offset(0, 20).release().perform()
                ...
                if from_step is not None and to_step is not None and from_step > to_step:
                    ...
                    start_y_offset = 0
                    end_x_offset = -size2['width'] / 2 + 2   # Cạnh trái elem2
                    end_y_offset = 0
                ...
                # === TRƯỜNG HỢP THƯỜNG: X < Y → DÙNG LOGIC HIỆN TẠI (theo hướng) ===
                    ...
                    if abs(delta_y) > abs(delta_x):
                        ...
                        if delta_y > 0:
                            ...
                            end_x_offset = 0
                            end_y_offset = -size2['height'] / 2 + 10
                        else:
            ...
            for i, step in enumerate(steps):
                ...
                while retry_attempt < max_retries and not success:
                    try:
                        ...
                        success = True
                        log(f"[SUCCESS] Step {current_step} thành công!")

                    except Exception as e:
                        log(f"[ERROR] Lỗi ở retry {retry_attempt}: {e}")
                        ...
                        if not (has_more_object or has_more_related):
                            log(f"[WARNING] Hết locator để thử cho step {current_step}")
                            break
                ...
                print(f"\nBước {current_step}: {status} | Thời gian: {step_duration:.2f}s", file=sys.stderr)
                ...
                if initial_snapshot_taken and action.lower() in ['click', 'connect', 'link', 'fill', 'enter']:
                    log(f"[FIND] Kiểm tra DOM sau action {action}...")
                    time.sleep(1)
                        ...
                        if hasattr(detect_new_g, 'find_new_elements_by_steps'):
                            ...
                            if classified:
                                ...
                                if new_count > previous_new_count:
                                    ...
                                    log("[UPDATE] Cập nhật mapping element...")
                                    ...
                                    for idx, step_num in enumerate(steps_created_elements_in_order):
                                        if idx < len(new_selectors):
                                            ...
                                            log(f"   [WARNING] Step {step_num} → Không đủ phần tử để map")
                                ...
                                # Nếu không tạo thêm phần tử mới
                                    ...
                                    for idx, step_num in enumerate(steps_created_elements_in_order):
                                        if idx < len(new_selectors):
                                            created_elements[step_num] = new_selectors[idx]
                                    log(f"[UPDATE] Mapping cập nhật: {created_elements}")
                                ...
                                log(f"[WARNING] find_new_elements_by_steps: {error}")
                                # Nếu có lỗi, bỏ qua logic kiểm tra new_count
                                continue

                        else:
                            log("[WARNING] Hàm find_new_elements_by_steps không tồn tại trong detect_new_g")
                            ...
                            if new_count > previous_new_count:
                                steps_created_elements_in_order.append(current_step)
                                log(f"[OK] Step {current_step} đã tạo phần tử mới! ({new_count - previous_new_count} element)")
                                log("[UPDATE] Cập nhật mapping element...")
                ...
                log(f"[SUCCESS] Hoàn thành step {current_step}")
...
if __name__ == "__main__":
    ...
    test_steps = [
        'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__172__25__20251129.png]',
        'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__172__25__20251129.png]',
        'Move the element created in step 2 right',
        'Connect the element created in step 1 to the element created in step 2',


        # 'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__172__25__20251129.png]',
        # 'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__173__25__20251129.png]',
        # 'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__174__25__20251129.png]',
        # 'Click on [http://localhost:8123/uploads/stepImg/objectImg/objectImg__175__25__20251129.png]',
        


        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__172__25__20251129.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__173__25__20251129.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__174__25__20251129.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__175__25__20251129.png

        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__166__25__20251127.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__167__25__20251127.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__168__25__20251127.png
        # http://localhost:8123/uploads/stepImg/objectImg/objectImg__169__25__20251125.png
