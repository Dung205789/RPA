...
public class GeneratedTest {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 15; i++) { actions.keyDown(Keys.SHIFT).sendKeys(Keys.ARROW_DOWN).keyUp(Keys.SHIFT).perform(); }
        // click D:\SVG_Agent\documents\RPA_drawio\shape_icons\cylinder.png (a, score 0.71)
        for (int i = 0; i < 15; i++) { actions.keyDown(Keys.SHIFT).sendKeys(Keys.ARROW_DOWN).keyUp(Keys.SHIFT).perform(); }
        for (int i = 0; i < 15; i++) { actions.keyDown(Keys.SHIFT).sendKeys(Keys.ARROW_DOWN).keyUp(Keys.SHIFT).perform(); }
        for (int i = 0; i < 15; i++) { actions.keyDown(Keys.SHIFT).sendKeys(Keys.ARROW_DOWN).keyUp(Keys.SHIFT).perform(); }
        for (int i = 0; i < 15; i++) { actions.keyDown(Keys.SHIFT).sendKeys(Keys.ARROW_DOWN).keyUp(Keys.SHIFT).perform(); }
        ...
        actions.sendKeys("Show the form").perform();   // into element created in step 7
        actions.doubleClick(cell).perform();   // element created in step 10
        actions.sendKeys("Validate the email").perform();   // into element created in step 10
        actions.doubleClick(cell).perform();   // element created in step 11
        actions.sendKeys("Read user input").perform();   // into element created in step 11
        actions.doubleClick(cell).perform();   // element created in step 14
        actions.sendKeys("Create the record").perform();   // into element created in step 14
        actions.doubleClick(cell).perform();   // element created in step 19
        actions.sendKeys("Account active").perform();   // into element created in step 19
